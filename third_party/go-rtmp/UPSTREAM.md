...
| 4 | `conn_state.go`, `chunk_streamer.go` (y el hunk de `stream.go` dentro de `0001-write-timeout.diff`) | `StreamControlState.chunkSize` se escribía desde la goroutine que llama a `Stream.CreateStream` mientras la goroutine del planificador de escritura lo leía en CADA chunk (`ChunkStreamer.writeChunk`): `DATA RACE`. No saltaba antes porque en el camino de siempre la única escritura anterior a `createStream` es `connect`, que espera respuesta del servidor; en cuanto se manda algo sin esperar —justo lo que hace `PreCommands` con `releaseStream` y `FCPublish`— la carrera aparece, y de perder esa carrera saldría un chunk escrito con un tamaño que el otro extremo todavía no conoce. `chunkSize` pasa a `atomic.Uint32` y todas las lecturas van por `ChunkSize()`. | `TestPreCommandsGoThroughTheControlStreamBeforeCreateStream` con `-race` (`internal/rtmpio/publisher_test.go`) |
